...
namespace Xtensive.Storage.Rse.Compilation.Expressions
{
    ...
    where TResult : class
  {
    ...
    protected virtual ReadOnlyCollection<TResult> VisitBindingList(ReadOnlyCollection<MemberBinding> original)
    {
      var results = new List<TResult>();
      for (int i = 0, n = original.Count; i < n; i++) {
        TResult p = VisitBinding(original[i]);
        results.Add(p);
      }
      return results.AsReadOnly();
    }
    ...
    protected virtual ReadOnlyCollection<TResult> VisitElementInitializerList(ReadOnlyCollection<ElementInit> original)
    {
      var results = new List<TResult>();
      for (int i = 0, n = original.Count; i < n; i++) {
        TResult p = VisitElementInitializer(original[i]);
        results.Add(p);
      }
      return results.AsReadOnly();
    }
