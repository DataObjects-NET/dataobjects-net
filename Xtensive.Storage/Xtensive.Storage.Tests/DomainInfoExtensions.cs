  ...
  public static class DomainInfoExtensions
  {
    ...
    private static void Dump(this IndexInfo target, int indent)
    {
      ...
      foreach (var columnGroup in target.ColumnGroups) {
        columnGroup.Dump(indent + 2);
      }
    }

    private static void Dump(this ColumnGroup target, int indent)
    {
      WriteLine(indent, "Type: " + target.Type.Name);
      WriteLine(indent, "Columns: " + target.Columns.Select(c => c.Name).ToCommaDelimitedString());
    }
