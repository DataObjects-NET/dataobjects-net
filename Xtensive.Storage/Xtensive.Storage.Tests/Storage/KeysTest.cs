  ...
  public class KeysTest : AutoBuildTest
  {
    ...
    public void ResolveKeyTest()
    {
      using (Session.Open(Domain))
      {
        using (var t = Transaction.Open())
        {
          var descriptor = Domain.Model.Types[typeof (Test)].Hierarchy.KeyProviderInfo.KeyTupleDescriptor;
    ...
    public void StoreKeyTest()
    {
      using (Session.Open(Domain)) {
        using (var t = Transaction.Open()) {
          ...
          var appleEntity2 = Query.SingleOrDefault<Apple>(a);
          var appleEntity3 = Query.SingleOrDefault<Apple>((object)a);
