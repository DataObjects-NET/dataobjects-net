...
using System.Linq;
using Xtensive.Storage.Tests.Upgrade.SkipUpgradeTestModel;

namespace Xtensive.Storage.Tests.Upgrade.SkipUpgradeTestModel
{
  ...
  public class SkipUpgradeTest
  {
    ...
    public void CombinedTest()
    {
      ...
      using (var initialDomain = Domain.Build(initialConfiguration)) {
        ...
        using (Session.Open(initialDomain))
      ...
      using (var testedDomain = Domain.Build(testedConfiguration)) {
        ...
        using (Session.Open(testedDomain))
        using (var ts = Transaction.Open()) {
