...
//        EnumerateAll(query);
      ...
      foreach (var value in values) {
        ...
        if (value==null || ((GetMemberType(value.GetType())==MemberType.Primitive
          ...
          foreach (var val in exactValue) {
            ...
            keyAttribute.Value = KeyToString(value.GetType().GetProperty(WellKnown.KeyFieldName).GetValue(value, null));
      ...
      if (property==null) {
        ...
        if (property.PropertyType.IsGenericType &&
          ...
          case MemberType.Entity:
            ...
            valueAttribute.Value = (entityValue!=null) ? entityValue.GetType().GetProperty(WellKnown.KeyFieldName).GetValue(entityValue, null).ToString() : "NULL";
            node.Attributes.Append(valueAttribute);
            break;
    ...
    private string KeyToString(object key)
    {
      ...
      case MemberType.Anonymous:
        foreach (var info in properties) {
          if (ValueIsForOutput(info)) {
            ...
            if (GetMemberType(info.PropertyType)==MemberType.Entity)
              str.Append((propertyValue!=null) ? info.PropertyType.GetProperty(WellKnown.KeyFieldName).GetValue(propertyValue, null).ToString() : "NULL");
        ...
        break;
      case MemberType.Entity:
        str.Append(key.GetType().GetProperty(WellKnown.KeyFieldName).GetValue(key, null).ToString());
        break;
