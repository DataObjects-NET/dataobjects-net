...
using Xtensive.Orm.Operations.Tests.OperationOrderTestModel;
...
namespace Xtensive.Orm.Operations.Tests
{
  ...
  public class OperationOrderTest : AutoBuildTest
  {
    ...
    {
      Assert.That(Domain.Services.Get<IOperationFactory>(), Is.Not.Null);

      using (var session = Domain.OpenSession()) {
        Assert.That(session.Operations, Is.InstanceOf<OperationRegistry>());
        BindEvents(session);
