...
using Xtensive.Orm.Operations.Internal;
...
namespace Xtensive.Orm.Operations
{
  ...
  public sealed class OperationFactory : IOperationFactory
  {
    ...
    /// <summary>
    /// Creates instance of <see cref="Operations.EntityFieldSetOperation"/>.
    /// </summary>
    /// <param name="keys">Keys of removed entites.</param>
    /// <returns>Created instance.</returns>
    ...
    /// Creates instance of <see cref="Operations.EntitiesRemoveOperation"/>.
    /// </summary>
    /// <param name="key">Key of EntitySet owner.</param>
    /// <param name="field">EntitySet fields.</param>
    /// <returns>Created instance.</returns>
    ...
    /// <param name="key">Key of EntitySet owner.</param>
    /// <param name="field">EntitySet fields.</param>
    /// <param name="itemKey">Key of item to add to the EntitySet</param>
    /// <returns>Created instance.</returns>
    ...
    /// <summary>
    /// Creates instance of <see cref="Operations.EntitySetItemRemoveOperation"/>.
    /// </summary>
    /// <param name="key">Key of EntitySet owner.</param>
    /// <param name="field">EntitySet fields.</param>
    /// <param name="itemKey">Key of item to remove from the EntitySet.</param>
    /// <returns>Created instance.</returns>
    ...
    /// Creates instance of <see cref="Operations.EntitiesRemoveOperation"/>.
    /// </summary>
    /// <param name="prepareAction">Keys of removed entites.</param>
    /// <param name="executeAction"></param>
    /// <param name="arguments"></param>
    /// <returns>Created instance.</returns>
  ...
  /// </summary>
  [ServiceAttribute(typeof(IOperationRegistry), Singleton = true)]
  public sealed class OperationRegistry : IOperationRegistry
  {
    ...
    public void RegisterOperation(IOperation operation, bool isStarted)
    {
      var scope = GetCurrentOperationRegistrationScope();
      if (scope == null)
        return;
      if (scope.Operation != null)
        throw new InvalidOperationException("Only one operation can be registered in each scope.");
      (operation as Operation).Type = scope.OperationType;
    ...
    public void NotifyOperationStarting(bool throwIfNotRegistered)
    {
      ...
      if (operation == null) {
        if (throwIfNotRegistered)
          throw new InvalidOperationException("Operation isn't registered yet. See Session.Operations.RegisterOperations().");
        else
          return;
      }
      if (scope.IsOperationStarted)
        throw new InvalidOperationException("OperatonStarted is alerady called for this operation.");
      scope.IsOperationStarted = true;
      if (scope.IsOutermost)
        NotifyOutermostOperationStarting(operation);
      else
    ...
    /// </summary>
    /// <param name="operation">The operation to register.</param>
    public void RegisterUndoOperation(IOperation operation)
    {
      var scope = GetCurrentOperationRegistrationScope();
      if (scope == null)
        return;
    ...
    /// </summary>
    /// <returns>An <see cref="IDisposable"/> object enabling the logging back on its disposal.</returns>
    public SystemOperationRegistrationScope DisableSystemOperationRegistration() => new SystemOperationRegistrationScope(this, false,
      static (reg, newState) =>  ((OperationRegistry) reg).IsSystemOperationRegistrationEnabled = newState);
    ...
    public SystemOperationRegistrationScope EnableSystemOperationRegistration() => new SystemOperationRegistrationScope(this, true,
      static (reg, newState) => ((OperationRegistry) reg).IsSystemOperationRegistrationEnabled = newState);
      ...
      try {
        ...
        if (!scope.IsOperationStarted) {
          if (scope.IsCompleted)
            throw new InvalidOperationException("Operation isn't marked as started. See Session.Operations.OperationStarted() method.");
          else
    ...
    private void NotifyOutermostOperationStarting(IOperation operation)
    {
      if (OutermostOperationStarting != null && IsRegistrationEnabled)
        OutermostOperationStarting(this, new OperationEventArgs(operation));
    }

    private void NotifyOutermostOperationCompleted(IOperation operation, bool isCompleted)
    {
    ...
    private void NotifyNestedOperationStarting(IOperation operation)
    {
      if (NestedOperationStarting != null && IsRegistrationEnabled)
        NestedOperationStarting(this, new OperationEventArgs(operation));
    }

    private void NotifyNestedOperationCompleted(IOperation operation, bool isCompleted)
    {
    ...
    private void NotifyUndoOperation(IOperation operation)
    {
      if (IsUndoOperationRegistrationEnabled)
    ...
    private OperationRegistrationScope GetCurrentOperationRegistrationScope()
    {
      ...
      if (scope == null)
        throw new InvalidOperationException("No operation registration scope. Use Session.OperationRegistry.BeginRegistration method to open it.");
