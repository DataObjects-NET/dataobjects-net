  ...
  internal class ReferenceFields : AutoBuildTest
  {
    ...
    public void InsertClientSideReferenceField()
    {
      ...
      using (var trx = session.OpenTransaction()) {
        ...
        var key = session.Query.Insert(() => new Foo(session) { Bar = bar, Name = "test" });
    ...
    public void InsertClientSideReferenceField2()
    {
      ...
      using (var trx = session.OpenTransaction()) {
        ...
        var key = session.Query.Insert(() => new Foo2(null, 0, null) { Bar = bar, Id = 0, Id2 = "test", Name = "test" });
    ...
    public void UpdateServerSideReferenceField()
    {
      ...
      using (var trx = session.OpenTransaction()) {
        ...
        _ = session.Query.All<Foo>().Set(a => a.Bar, (Bar) null).Update();
        Assert.That(foo.Bar, Is.Null);
        session.AssertCommandCount(
          Is.EqualTo(1),
          () =>
    ...
    public void UpdateServerSideReferenceField2()
    {
      using (var session = Domain.OpenSession())
      using (var trx = session.OpenTransaction()) {
        var date = FixDateTimeForPK(DateTime.Now);
        var id = Guid.NewGuid();
        ...
        _ = session.Query.All<Foo2>().Set(a => a.Bar, (Bar2) null).Update();
        Assert.That(foo.Bar, Is.Null);
        session.AssertCommandCount(
          Is.EqualTo(1),
          () =>
    ...
    public void UpdateClientSideReferenceField()
    {
      ...
      using (var trx = session.OpenTransaction()) {
        ...
        _ = session.Query.All<Foo>().Set(a => a.Bar, bar).Update();
        Assert.That(bar, Is.EqualTo(foo.Bar));
        _ = session.Query.All<Foo>().Set(a => a.Bar, (Bar) null).Update();
        Assert.That(foo.Bar, Is.Null);
        _ = session.Query.All<Foo>().Update(a => new Foo(session) { Bar = bar });
        Assert.That(bar, Is.EqualTo(foo.Bar));
        _ = session.Query.All<Foo>().Update(a => new Foo(session) { Bar = null });
        Assert.That(foo.Bar, Is.Null);
        trx.Complete();
      }
    }
    ...
    public void UpdateClientSideReferenceField2()
    {
      ...
      using (var trx = session.OpenTransaction()) {
        ...
        var foo = new Foo2(session, 1, "1");
        var bar2 = new Bar2(session, DateTime.Now, Guid.NewGuid());
        _ = session.Query.All<Foo2>().Set(a => a.Bar, bar).Update();
        Assert.That(bar, Is.EqualTo(foo.Bar));
        _ = session.Query.All<Foo2>().Set(a => a.Bar, (Bar2) null).Update();
        Assert.That(foo.Bar, Is.Null);
        _ = session.Query.All<Foo2>().Update(a => new Foo2(null, 0, null) { Bar = bar });
        Assert.That(bar, Is.EqualTo(foo.Bar));
        _ = session.Query.All<Foo2>().Update(a => new Foo2(null, 0, null) { Bar = null });
        Assert.That(foo.Bar, Is.Null);
        trx.Complete();
      }
    }
