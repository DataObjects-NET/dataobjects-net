  ...
  public class BatchingOfVersionizedEntitiesTest : AutoBuildTest
  {
    ...
    public void VersionedEntityUpdateWithVersionCheckTest03()
    {
      ...
      using (var transaction = session.OpenTransaction()) {
        ...
        _ = session.Query.CreateDelayedQuery(q => q.All<Store>().Take(() => 1));
        _ = session.Query.CreateDelayedQuery(q => q.All<Store>().Take(() => 2));
        _ = session.Query.CreateDelayedQuery(q => q.All<Store>().Take(() => 3));
        _ = session.Query.CreateDelayedQuery(q => q.All<Store>().Take(() => 4));
    ...
    public async Task VersionedEntityUpdateWithoutVersionCheckAsyncTest03()
    {
      ...
      using (var transaction = session.OpenTransaction()) {
        ...
        using (counter.Attach()) {
          (await session.Query.All<Author>().ExecuteAsync()).Run();
        }
