  ...
  public abstract class AsyncQueriesBaseTest : AutoBuildTest
  {
    ...
    private void PopulateCourses(Session session)
    {
      using (var transaction = session.OpenTransaction()) {
        ...
        var currentCourse = new Course(session) {
          Name = "Some course",
          Year = DateTime.Now.Year,
          Speciality = speciality
        };
        var previousCourse = new Course(session) {
          Name = "Some course",
    ...
    private void PopulateGroups(Session session)
    {
      using (var transaction = session.OpenTransaction()) {
        ...
        foreach (var course in courses) {
          new Group {Name = course.Year==DateTime.Now.Year ? "Current year group" : "Last year group"}
            .Courses.Add(course);
        }
