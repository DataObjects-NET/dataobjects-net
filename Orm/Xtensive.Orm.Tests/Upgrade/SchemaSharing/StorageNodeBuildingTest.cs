...
namespace Xtensive.Orm.Tests.Upgrade.SchemaSharing
{
  ...
  {
    private readonly Dictionary<UpgradeStage, ModelMapping> mappings = new Dictionary<UpgradeStage, ModelMapping>();
    ...
    public void Reset() => mappings.Clear();
  }
  ...
  public class StorageNodeBuildingTest : AutoBuildTest
  {
    ...
    [Test]
    public void RecreateTestWithoutSharingTest() => RunTest(DomainUpgradeMode.Recreate, false);

    [Test]
    public void RecreateTestWithSharingTest() => RunTest(DomainUpgradeMode.Recreate, true);

    [Test]
    public void PerformSafelyWithoutChangesWithoutSharingTest() => RunTest(DomainUpgradeMode.PerformSafely, false, false);

    [Test]
    public void PerformSafelyWithoutChangesWithSharingTest() => RunTest(DomainUpgradeMode.PerformSafely, true, false);

    [Test]
    public void PerformSafelyWithChangesWithoutSharing() => RunTest(DomainUpgradeMode.PerformSafely, false, true);

    [Test]
    public void PerformSafelyWithChangesWithSharing() => RunTest(DomainUpgradeMode.PerformSafely, true, true);

    [Test]
    public void PerformWithoutChangesWithoutSharingTest() => RunTest(DomainUpgradeMode.Perform, false, false);

    [Test]
    public void PerformWithoutChangesWithSharingTest() => RunTest(DomainUpgradeMode.Perform, true, false);

    [Test]
    public void PerformWithChangesWithSharingTest() => RunTest(DomainUpgradeMode.Perform, true, true);

    [Test]
    public void PerformWithChangesWithoutSharingTest() => RunTest(DomainUpgradeMode.Perform, false, true);
    ...
    [Test]
    public void LegacySkipWithountSharingTest() => RunTest(DomainUpgradeMode.LegacySkip, false);
    ...
    private void RunTest(DomainUpgradeMode upgradeMode, bool shareStorageSchemaOverNodes, bool withModelChanges = false)
    {
      ...
      using (var initialDomain = BuildDomain(configuration)) {
        ...
        if (isMultinode) {
          ...
          _ = initialDomain.StorageNodeManager.AddNode(nodeConfiguration);
    ...
    private void ValidateFinalMappings(Domain domain, ModelMapping mapping)
    {
      ...
      foreach (var schemaNode in mapping.GetAllSchemaNodes()) {
        ...
        if (readingNamesDenied) {
          _ = Assert.Throws<InvalidOperationException>(() => { var schemaName = schemaNode.Schema.Name; });
          _ = Assert.Throws<InvalidOperationException>(() => { var schemaDbName = schemaNode.Schema.DbName; });
          _ = Assert.Throws<InvalidOperationException>(() => { var catalogName = schemaNode.Schema.Catalog.Name; });
          _ = Assert.Throws<InvalidOperationException>(() => { var catalogDbName = schemaNode.Schema.Catalog.DbName; });
        }
        else {
