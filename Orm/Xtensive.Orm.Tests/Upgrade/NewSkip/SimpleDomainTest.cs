...
{
  /// <summary>
  /// Test for simple domain which includes only one database and one default scheme
  /// </summary>
  public abstract class SimpleDomainTest : SkipBuildingTestBase
  {
    ...
    [Test]
    [Ignore("Due to it's inconsistent for now")]
    public void CatalogsComparisonTest()
    {
      Assert.That(ExtractionResultKeepers.Count, Is.EqualTo(2));
      var first = ExtractionResultKeepers.First();
      var second = ExtractionResultKeepers.Last();
      Assert.That(first.FinalResults.Count, Is.EqualTo(0));
      Assert.That(second.InitialResults.Count, Is.EqualTo(0));
    }
  ...
  public class SimpleDomainWithoutForeignKeysTest : SimpleDomainTest
  {
  ...
  public class SimpleDomainHierarchyForeignKeysTest : SimpleDomainTest
  {
    protected override ForeignKeyMode GetForeignKeyMode()
    {
  ...
  public class SimpleDomainReferenceForeignKeysTest : SimpleDomainTest
  {
    protected override ForeignKeyMode GetForeignKeyMode()
    {
  ...
  public class SimpleDomainAllForeignKeysTest : SimpleDomainTest
  {
    protected override ForeignKeyMode GetForeignKeyMode()
    {
