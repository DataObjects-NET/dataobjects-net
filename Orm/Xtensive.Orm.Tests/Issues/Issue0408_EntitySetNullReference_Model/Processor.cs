...
namespace Xtensive.Orm.Tests.Issues.Issue0408_EntitySetNullReference_Model
{
  ...
  public abstract class Processor : Entity
  {
    ...
    {
      TestLog.Debug($"Propagating to document {destinationDocument} all groups of document {sourceDocument}.");
      ...
      if (qr.Count() > 0)
      {
        foreach (DocumentLink link in qr)
        {
          ...
          TestLog.Debug($"Document/ID={destinationDocument.Id} inherits a link from Document/ID={sourceDocument.Id}. Head of the group is Document/ID={link.LinkSource.Id}. (LinkSemantic={link.LinkSemantic})");
        }
      }
      else
      {
        TestLog.Warning($"Creating a new group from this document because no links point to it. (Document is {sourceDocument})"); 
