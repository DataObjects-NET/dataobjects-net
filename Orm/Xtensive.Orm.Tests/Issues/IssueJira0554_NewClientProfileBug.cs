  ...
  public class IssueJira0554_NewClientProfileBug : AutoBuildTest
  {
    ...
    public void Test01()
    {
      ...
      using(var session = Domain.OpenSession(new SessionConfiguration(SessionOptions.ClientProfile))) {
        ...
        var testA1 = testB.TestA;
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
        testB.TestA = null;
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testB.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
      }
    }
    ...
    public void Test04()
    {
      ...
      using (var session = Domain.OpenSession(new SessionConfiguration(SessionOptions.ClientProfile))) {
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA2.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA2.State).Count);
        testB.TestA = testA2;
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testB.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA2.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetAddedReferences(testA2.State).Count);
        testB.TestA = testA1;
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testB.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
    ...
    public void Test05()
    {
      ...
      using (var session = Domain.OpenSession(new SessionConfiguration(SessionOptions.ClientProfile))) {
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testD1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testD1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA2.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA2.State).Count);
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testD1.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
        Assert.AreEqual(testD1.State, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Keys.First());
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA2.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetAddedReferences(testA2.State).Count);
        Assert.AreEqual(testD1.State, session.NonPairedReferencesRegistry.GetAddedReferences(testA2.State).Keys.First());
        session.SaveChanges();
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testD1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testD1.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA1.State).Count);
    ...
    public void Test07()
    {
      ...
      using (var transaction = session.OpenTransaction()) {
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(entity.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State)[entity.State]);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State).Count);
      }
    }
    ...
    public void Test08()
    {
      ...
      using (var transaction = session.OpenTransaction()) {
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(entity.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State).Count);
        Assert.AreEqual(2, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State)[entity.State]);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State).Count);
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State).Count);
      }
    }
    ...
    public void Test09()
    {
      ...
      using (var transaction = session.OpenTransaction()) {
        ...
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State)[entity.State]);
        entity.FirstTestA = null;
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetRemovedReferences(entity.State).Count);
        Assert.AreEqual(0, session.NonPairedReferencesRegistry.GetAddedReferences(testA.State).Count);
        Assert.AreEqual(1, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State).Count);
        Assert.AreEqual(2, session.NonPairedReferencesRegistry.GetRemovedReferences(testA.State)[entity.State]);
      }
    }
