...
namespace Xtensive.Orm.Tests.Core.DotNetFramework
{
  ...
  public class CoreFeaturesTest
  {
    ...
    {
      using (warmup ? EmptyDisposable() : TestLog.InfoRegion("int operations")) {
        int j = 0;
        using (warmup ? EmptyDisposable() : TestLog.InfoRegion("Unchecked")) unchecked{
        ...
        using (warmup ? EmptyDisposable() : TestLog.InfoRegion("Checked")) checked{
          using (warmup ? (IDisposable)new Disposable(delegate { }) : 
            new Measurement("Empty loop", MeasurementOptions.Log, count)) {
            for (int i = 0; i < count; i++) {
            }
    ...
    {
      using (warmup ? EmptyDisposable() : TestLog.InfoRegion("long operations")) {
        long j = 0;
        using (warmup ? EmptyDisposable() : TestLog.InfoRegion("Unchecked")) unchecked{
          using (warmup ? (IDisposable)new Disposable(delegate { }) : 
            new Measurement("Empty loop", MeasurementOptions.Log, count)) {
    ...
    {
      using (warmup ? EmptyDisposable() : TestLog.InfoRegion("double operations")) {
        double j = 0;
        using (warmup ? EmptyDisposable() : TestLog.InfoRegion("Unchecked")) unchecked{
          using (warmup ? (IDisposable) new Disposable(delegate { }) :
    ...
    private void TestCast(int size, bool warmup)
    {
      ...
      using (warmup ? EmptyDisposable() : TestLog.InfoRegion("Casts")) {
        using (warmup ? (IDisposable) new Disposable(delegate { }) :
