...
{
  public class CrossDatabaseTableMoveTest : MultidatabaseTest
  {
    [Test]
    ...
    private void BuildInitialDomain()
    {
      ...
      r.Map(typeof (V1.Namespace1.Renamed1).Namespace).ToDatabase(Database1Name);
      r.Map(typeof (V1.Namespace2.Renamed2).Namespace).ToDatabase(Database2Name);
    ...
    private void BuildUpgradedDomain()
    {
      ...
      var r = config.MappingRules;
      r.Map(typeof (V2.Namespace1.Renamed2).Namespace).ToDatabase(Database1Name);
      r.Map(typeof (V2.Namespace2.Renamed1).Namespace).ToDatabase(Database2Name);
      var domain = Domain.Build(config);
      using (domain) {
        using (var session = domain.OpenSession())
        using (var tx = session.OpenTransaction()) {
