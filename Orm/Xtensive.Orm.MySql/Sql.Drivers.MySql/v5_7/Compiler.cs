...
namespace Xtensive.Sql.Drivers.MySql.v5_7
{
  internal class Compiler : SqlCompiler
  {
    ...
    {
      using (context.EnterScope(node)) {
        var leftNeedsParenthesis = node.Left is SqlSelect sL
          && (sL.OrderBy.Count > 0 || sL.HasLimit || sL.Lock != SqlLockType.Empty);
        var rightNeedsParenthesis = node.Left is SqlSelect sR
    ...
    protected virtual SqlUserFunctionCall TimeAddInterval(SqlExpression time, SqlExpression interval)
    {
