...
namespace Xtensive.Orm.Validation
{
    ...
    public class ValidationContext
    {
        ...
        public void Validate()
        {
            ...
            try {
                while (registry!=null) {
                    var copy = registry;
                    registry = null;
                    foreach (var pair in (IEnumerable<Pair<IValidationAware, Action<IValidationAware>>>) copy) {
                        try {
                            if (pair.Second == null)
                                pair.First.OnValidate();
                            else if (
                                !copy.Contains(new Pair<IValidationAware, Action<IValidationAware>>(pair.First, null)))
        ...
        /// </param>
        protected internal virtual void EnqueueValidate(IValidationAware target,
            Action<IValidationAware> validationDelegate)
        {
