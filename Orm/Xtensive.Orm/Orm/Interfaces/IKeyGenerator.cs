...
namespace Xtensive.Orm
{
  ...
  public interface IKeyGenerator
  {
    ...
    void Initialize(Domain ownerDomain, TupleDescriptor keyTupleDescriptor);

    /// <summary>
    /// Generates new key for the specified <paramref name="keyInfo"/>.
    /// </summary>
    /// <param name="keyInfo"><see cref="KeyInfo"/> that defines key to generate.</param>
    /// <param name="session">Current session.</param>
    /// <returns></returns>
    Tuple GenerateKey(KeyInfo keyInfo, Session session);
  }
}