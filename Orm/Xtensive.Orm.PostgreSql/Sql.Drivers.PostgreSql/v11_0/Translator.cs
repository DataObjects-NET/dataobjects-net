...
namespace Xtensive.Sql.Drivers.PostgreSql.v11_0
{
  internal class Translator : SqlTranslator
  {
    ...
    protected virtual void InitFunctionTypeTranslations()
    {
      ...
      FunctionTypeTranslations.Add(SqlFunctionType.CurrentDate, "date_trunc('day', clock_timestamp())");
      FunctionTypeTranslations.Add(SqlFunctionType.CurrentTimeStamp, "clock_timestamp()");
      ...
      switch (section) {
        ...
        case CreateIndexSection.ColumnsExit:
          if (index.IsFullText) {
            ...
            TranslateFullTextVector(context, (FullTextIndex) index);
          }
          _ = output.Append(")");
          break;
    ...
    internal protected virtual void TranslateFullTextVector(SqlCompilerContext context, FullTextIndex index)
    {
      var output = context.Output;
      ...
      foreach (var languageGroup in languageGroups) {
        ...
        foreach (var language in languageGroup) {
          ...
          isFirstInner = false;
          _ = output.Append("(");
          TranslateIdentifier(output, language.Name);
          _ = output.Append(")::text");
    ...
    public override void Translate(IOutput output, SqlDateTimeOffsetPart part)
    {
      switch (part) {
        ...
        case SqlDateTimeOffsetPart.Offset:
          _ = output.Append("TIMEZONE");
          break;
        case SqlDateTimeOffsetPart.TimeZoneHour:
          _ = output.Append("TIMEZONE_HOUR");
          break;
        case SqlDateTimeOffsetPart.TimeZoneMinute:
          _ = output.Append("TIMEZONE_MINUTE");
          break;
        default:
          base.Translate(output, part);
          break;
    ...
    /// <summary>
    /// Translates .NET common types to SQL counterpart
    /// </summary>
    ...
    protected virtual string TranslateClrType(Type type)
    {
      switch (Type.GetTypeCode(type)) {
        ...
        case TypeCode.DateTime: {
          var valueType = Driver.TypeMappings.GetMapping(type).MapType();
        ...
        default:
          if (type == WellKnownTypes.TimeSpanType || type == WellKnownTypes.GuidType) {
            var valueType = Driver.TypeMappings.GetMapping(type).MapType();
