...
namespace Xtensive.Sql.Drivers.PostgreSql.v11_0
{
  internal class Extractor : Model.Extractor
  {
    ...
    protected virtual void BuildPgCatalogSchema(Schema schema)
    {
      ...
      t = schema.CreateTable("pg_sequence");
    ...
    {
      var context = CreateCatalogAndContext(catalogName, schemaNames);

      ExtractRoles(context, false).GetAwaiter().GetResult();
    ...
    private static ExtractionContext CreateCatalogAndContext(string catalogName, string[] schemaNames)
    {
    ...
    private async ValueTask ExtractRoles(ExtractionContext context, bool isAsync, CancellationToken token = default)
    {
      ...
      static void ReadUserData(DbDataReader dr, ExtractionContext context)
      {
    ...
    {
      context.CurrentUserIdentifier = GetMyUserSysId(context.CurrentUserSysId, isAsync: false, CancellationToken.None)
    ...
    {
      context.CurrentUserIdentifier = await GetMyUserSysId(context.CurrentUserSysId, isAsync: true, token).ConfigureAwait(false);
    ...
    protected static void CreateOidColumn(Table t, string name) =>
      t.CreateColumn(name, new SqlValueType(SqlType.Int64));
    ...
    protected void CreateInt8Column(Table t, string name) =>
      t.CreateColumn(name, new SqlValueType(SqlType.Int64));
    ...
    protected static ReferentialAction GetReferentialAction(char c)
    {
      return c switch {
        ...
        _ => ReferentialAction.NoAction,//a
      };
    }
    ...
    {
      return c switch {
        'f' => SqlMatchType.Full,//f
        _ => SqlMatchType.None,//u
      };
    }
    ...
    private async ValueTask<long> GetMyUserSysId(long mUserSysId, bool isAsync, CancellationToken token)
    {
      if (mUserSysId >= 0) {
        return mUserSysId;
      }

      var cmd = Connection.CreateCommand("SELECT usesysid FROM pg_user WHERE usename = user");
      if (isAsync) {
        await using (cmd.ConfigureAwait(false)) {
