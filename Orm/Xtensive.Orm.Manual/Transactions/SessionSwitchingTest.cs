      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            ...
            AssertEx.Throws<InvalidOperationException>(() => name = personA.ShortName);
    ...
    public void DenySwitchingOnPrimitiveFieldWriteTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) {// Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => personA.ShortName = "def");
    ...
    public void DenySwitchingOnReferenceFieldReadTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => passport = personA.Passport);
    ...
    public void DenySwitchingOnReferenceFieldWriteTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => personA.Passport = new Passport());
          }
        }
    ...
    public void DenySwitchingStructureReadTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => fullName = personA.FullName);
    ...
    public void DenySwitchingStructureWriteTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => personA.FullName = new FullName { FirstName = "def", LastName = "ghi" });
    ...
    public void DenySwitchingOnStructureFieldReadTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => lastName = personA.FullName.LastName);
    ...
    public void DenySwitchingOnStructureFieldWriteTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => personA.FullName.LastName = "def");
    ...
    public void DenySwitchingOnEntitySetAddTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, 
          // but allowed, since there is no running transaction
          _ = personA.Notifications.Add(notifications[0]);

          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => _ = personA.Notifications.Add(notifications[1]));

            // Blocking session switching check
            using (Session.Deactivate()) {
              _ = personA.Notifications.Add(notifications[2]);
            }
          }
        }
      }
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, 
          // but allowed, since there is no running transaction
          _ = personA.Notifications.Add(notification1);

          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => _ = personA.Notifications.Add(notification2));

            // Blocking session switching check
            using (Session.Deactivate()) {
              _ = personA.Notifications.Add(notification3);
            }
          }
        }
      }
    ...
    public void DenySwitchingOnEntitySetRemoveTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, 
          // but allowed, since there is no running transaction
          _ = personA.Notifications.Remove(notifications[0]);

          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => _ = personA.Notifications.Remove(notifications[1]));

            // Blocking session switching check
            using (Session.Deactivate()) {
              _ = personA.Notifications.Remove(notifications[2]);
            }
          }
        }
      }
    ...
    public void DenySwitchingOnEntitySetContainsTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, 
          // but allowed, since there is no running transaction
          _ = personA.Notifications.Contains(notifications[0]);

          using (var tx = Session.Demand().OpenTransaction()) {
            // Session switching (from sessionB to sessionA) will be detected & blocked here
            AssertEx.Throws<InvalidOperationException>(() => _ = personA.Notifications.Contains(notifications[1]));

            // Blocking session switching check
            using (Session.Deactivate()) {
              _ = personA.Notifications.Contains(notifications[2]);
            }
          }
        }
      }
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          ...
          using (var tx = Session.Demand().OpenTransaction()) {
            ...
            _ = Assert.Throws<InvalidOperationException>(() => personA.Passport = new Passport());
          }
        }
      }
    }
    ...
    public void AllowSwitchingOnEntitySetRemoveTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, but allowed
          using (var tx = Session.Demand().OpenTransaction()) {
            _ = personA.Notifications.Remove(notification);
          }
        }
      }
    }
    ...
    public void AllowSwitchingOnEntitySetContainsTest()
    {
      ...
      using (var tx1 = sessionA.OpenTransaction()) { // Open & activate
        ...
        using (var sessionB = Domain.OpenSession(sessionCfg)) { // Open & activate
          // Session switching (from sessionB to sessionA) will be detected here, but allowed
          using (var tx = Session.Demand().OpenTransaction()) {
            _ = personA.Notifications.Contains(notification);
          }
        }
      }
    }
