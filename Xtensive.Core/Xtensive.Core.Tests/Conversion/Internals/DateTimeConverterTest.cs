...
namespace Xtensive.Tests.Conversion
{
  ...
  public class DateTimeConverterTest : ConverterTestBase
  {
    ...
    public void Test()
    {
      Log.Info("{0}", DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss.fff %K", CultureInfo.InvariantCulture));
      ...
      for (int i = 0; i < iterationCount * 1; i++)
        Log.Info("{0}", generator.GetInstance(random).ToString("yyyy/MM/dd hh:mm:ss.fff %K", CultureInfo.InvariantCulture));
    }
