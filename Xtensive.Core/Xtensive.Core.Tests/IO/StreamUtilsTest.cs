...
namespace Xtensive.Core.Tests.IO
{
  ...
  {
    private readonly static string LargeFileName = typeof(StreamUtilsTest).GetShortName()+".BigFile.bin";
    ...
    public void WriteTo()
    {
      ...
      StreamUtils.WriteTo(source, destination, 4);
    ...
    public void WriteToEnd()
    {
      ...
      StreamUtils.WriteTo(source, destination);
    ...
    public void InstreamCopySmall()
    {
      ...
      StreamUtils.InstreamCopy(stream, 9, 100);
      for (int i = 0; i < buffer.Length; i++) {
    ...
    public void InstreamCopySmallForward()
    {
      ...
      StreamUtils.InstreamCopy(stream, 7, 3);
      for (int i = 0; i < buffer.Length; i++)
      {
        Assert.AreEqual(buffer[i], expectedResult[i]);
      }
    ...
    public void InstreamCopySmallReverse()
    {
      ...
      Stream stream = new MemoryStream(buffer);
      stream.Seek(8, SeekOrigin.Begin);
      StreamUtils.InstreamCopy(stream, 9, 3);
      for (int i = 0; i < buffer.Length; i++)
      {
        Assert.AreEqual(buffer[i], expectedResult[i]);
      }
    ...
    public void InstreamCopyLargeForward()
    {
      ...
      try {
        ...
        StreamUtils.InstreamCopy(stream, 0, bytesToCopy);
        stream.Flush();
        // Verify
        stream.Close();
        stream.Dispose();
