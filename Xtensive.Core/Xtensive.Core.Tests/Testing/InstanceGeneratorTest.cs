...
//      return np*np + (1-np)*(1-np)*p;
    ...
    {
      Log.Info("{0} random sequence, {1} items:", typeof(T).GetShortName(), size);
      using (new LogIndentScope()) {
        Log.Info("Expected probability: {0}, tolerance: {1}.", expectedShare, shareTolerance);
        ...
        Log.Info("Actual probability:   {0}.", equalityProbability);
