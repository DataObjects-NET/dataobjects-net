...
//      VMethodCallTest<Pair<Guid, Guid>>(0.2*speedFactor);
    ...
    private void MethodCallTest<T>(double speedFactor)
    {
      ...
      Log.Info("Regular call test:");
      Log.Info("  Type: {0}", typeof(T).GetShortName());
    ...
    private void GMethod1CallTest<T,T1>(double speedFactor)
    {
      ...
      // Real test
      iterations = (int)(IterationCount*speedFactor);
      Log.Info("Regular call test (1 generic argument: {0}):", typeof(T1).GetShortName());
      Log.Info("  Type: {0}", typeof(T).GetShortName());
      FastCache<T>.Value = null;
      using (new LogIndentScope()) {
        Cleanup();
    ...
    private void GMethod2CallTest<T,T1,T2>(double speedFactor)
    {
      ...
      // Real test
      iterations = (int)(IterationCount*speedFactor);
      Log.Info("Regular call test (2 generic arguments: {0}, {1}):", typeof(T1).GetShortName(), typeof(T2).GetShortName());
      Log.Info("  Type: {0}", typeof(T).GetShortName());
      FastCache<T>.Value = null;
      using (new LogIndentScope()) {
        Cleanup();
    ...
    private void GVMethod1CallTest<T,T1>(double speedFactor)
    {
      ...
      // Real test
      iterations = (int)(IterationCount*speedFactor);
      Log.Info("Virtual generic call test (1 generic argument: {0}):", typeof(T1).GetShortName());
      Log.Info("  Type: {0}", typeof(T).GetShortName());
      FastCache<T>.Value = null;
      using (new LogIndentScope()) {
        Cleanup();
    ...
    private void GVMethod2CallTest<T,T1,T2>(double speedFactor)
    {
      ...
      // Real test
      iterations = (int)(IterationCount*speedFactor);
      Log.Info("Virtual generic call test (2 generic arguments: {0}, {1}):", typeof(T1).GetShortName(), typeof(T2).GetShortName());
      Log.Info("  Type: {0}", typeof(T).GetShortName());
      FastCache<T>.Value = null;
      using (new LogIndentScope()) {
        Cleanup();
