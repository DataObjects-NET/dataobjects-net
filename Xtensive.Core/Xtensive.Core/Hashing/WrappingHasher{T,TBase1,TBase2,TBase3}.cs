...
namespace Xtensive.Hashing
{
  ...
  [Serializable]
  public abstract class WrappingHasher<T, TBase1, TBase2, TBase3> : HasherBase<T>
  {
    /// <summary>
      ...
      : base(provider)
    {
      ...
      if (!TypeHelper.IsFinal<TBase2>() && !(BaseHasher2.Hasher.Implementation is IFinalAssociate))
        BaseHasher2 = null;
      BaseHasher3 = provider.GetHasher<TBase3>();
      if (!TypeHelper.IsFinal<TBase3>() && !(BaseHasher3.Hasher.Implementation is IFinalAssociate))
        BaseHasher3 = null;
    }
  }
}