...
namespace Xtensive.Core.Collections
{
  ...
  public class FastCache<TKey, TItem, TCached> :
    ICache<TKey, TItem>
    where TCached: IIdentified<TKey>, IHasSize
  {
    ...
    private class CachedItem
    {
      ...
      public CachedItem(TCached value)
      {
        Value = value;
      }
    }
    ...
    {
      time++;
      if ((time >> timeShiftFactor) > items.Count)
    }
    ...
    public FastCache(int maxSize, Converter<TItem, TKey> keyExtractor)
    {
      if (maxSize <= 0)
    ...
    /// <param name="keyExtractor"><see cref="KeyExtractor"/> property value.</param>
    public FastCache(int maxSize, Converter<TItem, TKey> keyExtractor, Biconverter<TItem, TCached> cacheConverter)
