...
namespace Xtensive.TransactionLog.Tests
{
  ...
  public class ClassMembers
  {
    ...
    private readonly IFileNameFormatter<long> fileNameFormatter = new TestFileNameProvider();
    ...
    {
      SetUp();
      using (new TransactionLog<long>(logProvider, logName, fileNameFormatter, TimeSpan.FromSeconds(1), 1, null, ValueSerializerProvider.Default.GetSerializer<long>()))
      {
      }
    }
    ...
    public void CounterHasNoValue()
    {
      ...
      using (
        TransactionLog<long> log =
          new TransactionLog<long>(logProvider, logName, fileNameFormatter, TimeSpan.FromSeconds(1), 1, null, ValueSerializerProvider.Default.GetSerializer<long>()))
      {
