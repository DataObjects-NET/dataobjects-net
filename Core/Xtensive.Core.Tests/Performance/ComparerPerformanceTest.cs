...
//      StructTest<double>(1);
    ...
    public void TwoValuesTest<T>(int count)
    {
      ...
      Log.Info("Values comparison:");
      Log.Info("  Type: {0}, instances: {1} x 2, {2}", typeof(T).GetShortName(), o1, o2);
      using (new LogIndentScope()) {
    ...
    public void ArrayTest<T>(int length, int count, double equalityProbability)
    {
      ...
      Log.Info("Array comparison (equality probability = {0}):", (int)(equalityProbability*100));
      Log.Info("  Type: {0}, array length: {1}", typeof(T).GetShortName(), length);
      using (new LogIndentScope()) {
        TestHelper.CollectGarbage();
        if (!TestInfo.IsProfileTestRunning) {
